...
@Component({
  selector: 'app-jobperson-list',
  templateUrl: './jobperson-list.component.html',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class JobpersonListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {

  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(JobPersonWebApiService);
  ...
  title = signal('سمت های اشخاص')
  ClassName = signal('jobperson')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetJobPerson(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.JobPersons)
