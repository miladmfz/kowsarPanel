...
import { JobWebApiService } from 'src/app/features/accounting/services/TaarifPayeWebApi/JobWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class JobListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(JobWebApiService);
  ...
  GetData() {
    ...
    this.repo.GetJob(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Jobs)
