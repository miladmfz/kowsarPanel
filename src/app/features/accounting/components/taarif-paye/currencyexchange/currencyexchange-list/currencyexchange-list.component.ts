...
import { CurrencyExchangeWebApiService } from 'src/app/features/accounting/services/TaarifPayeWebApi/CurrencyExchangeWebApi.service';
@Component({
...
export class CurrencyexchangeListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CurrencyExchangeWebApiService);
  ...
  title = signal('ارز های خارجی')
  ClassName = signal('CurrencyExchange')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCurrencyExchange(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CurrencyExchanges)
