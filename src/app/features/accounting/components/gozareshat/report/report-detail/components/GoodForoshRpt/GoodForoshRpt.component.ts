...
export class GoodForoshRptComponent extends AgGridBaseComponent implements OnInit, OnDestroy {
  ...
  gridKey = signal('');
  records_Stack = signal<any[]>([])
  records_GridSchema = signal<any[]>([])

  columnChartData = signal<any[]>([])
  columnChartCategories = signal<any[]>([])

  GroupBy_show = signal(false)
  show_report_list = signal(true)


  modal_title = signal('')
  lable_report_view = signal('نمایش به صورت چارتی')
  ...
  togget_report_view() {
    if (this.show_report_list) {
      this.show_report_list.set(false)
      this.lable_report_view.set('نمایش به صورت لیستی')
      ...
      if (this.EditForm_SearchTarget.value.withGroupBy == "0") {
        this.columnChartCategories.set(this.records().map(item => item.GoodName).slice(0, 10))

      } else {
        const fieldName = this.EditForm_SearchTarget.value.GroupByField;

        this.columnChartCategories.set(this.records().map(item => item[fieldName]).slice(0, 10))

      }


    } else {
      this.show_report_list.set(true)
      this.lable_report_view.set('نمایش به صورت چارتی')
  ...
  private initColumns(): void {
    ...
    this.repo.GetAllGridSchemaReport('TGood')
      .pipe(
        catchError((_error) => {
      ...
      .subscribe((data: any) => {
        ...
        this.records_GridSchema.set(data?.GridSchemas ?? [])
