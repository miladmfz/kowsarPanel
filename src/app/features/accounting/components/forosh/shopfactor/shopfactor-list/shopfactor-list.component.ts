...
import { ShopFactorWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/ShopFactorWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class ShopfactorListComponent extends AgGridBaseComponent
  ...
  private readonly repo = inject(ShopFactorWebApiService);
  ...
  title = signal('فاکتور فروشگاهی')
  ClassName = signal('ShopFactor')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetShopFactor(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.ShopFactors)
