...
import { CafeResturantFactorWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/CafeResturantFactorWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-cafe-resturant-factor-list',
  templateUrl: './cafe-resturant-factor-list.component.html',
})
export class CafeResturantFactorListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CafeResturantFactorWebApiService);
  ...
  title = signal('فاکتور کافه و رستوران')
  ClassName = signal('Factor')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCafeResturantFactor(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Factors)
