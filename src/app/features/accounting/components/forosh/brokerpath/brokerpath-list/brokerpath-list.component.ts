...
import { BrokerPathWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/BrokerPathWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-brokerpath-list',
  templateUrl: './brokerpath-list.component.html',
})
export class BrokerpathListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(BrokerPathWebApiService);
  ...
  title = signal('مسیر بازاریاب')
  ClassName = signal('BrokerPath')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetBrokerPath(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.BrokerPaths)
