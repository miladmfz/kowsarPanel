...
import { PersonEtebarWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/PersonEtebarWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-personetebar-list',
  templateUrl: './personetebar-list.component.html',
})
export class PersonetebarListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(PersonEtebarWebApiService);
  ...
  title = signal('اعتباردهی به مشترکین')
  ClassName = signal('PersonEtebar')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetPersonEtebar(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.PersonEtebars)
