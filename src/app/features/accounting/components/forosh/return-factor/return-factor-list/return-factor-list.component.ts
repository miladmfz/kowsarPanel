...
import { ReturnFactorWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/ReturnFactorWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-return-factor-list',
  templateUrl: './return-factor-list.component.html',
})
export class ReturnFactorListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(ReturnFactorWebApiService);
  ...
  title = signal('مرجوع از فروش')
  ClassName = signal('ReturnFactor')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetReturnFactor(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.ReturnFactors)
