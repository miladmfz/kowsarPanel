...
import { CustomerMandehWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/CustomerMandehWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-customer-mandeh-list',
  templateUrl: './customer-mandeh-list.component.html',
})
export class CustomerMandehListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CustomerMandehWebApiService);
  ...
  title = signal('مرور حساب مشتریان')
  ClassName = signal('CustomerMandeh')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCustomerMandeh(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CustomerMandehs)
