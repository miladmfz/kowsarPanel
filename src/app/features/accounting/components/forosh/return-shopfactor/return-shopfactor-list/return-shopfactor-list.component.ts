...
import { ReturnShopFactorWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/ReturnShopFactorWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-return-shopfactor-list',
  templateUrl: './return-shopfactor-list.component.html',
})
export class ReturnShopfactorListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(ReturnShopFactorWebApiService);
  ...
  title = signal('فاکتور مرجوعی فروشگاهی')
  ClassName = signal('ReturnShopFactor')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetReturnShopFactor(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.ReturnShopFactors)
