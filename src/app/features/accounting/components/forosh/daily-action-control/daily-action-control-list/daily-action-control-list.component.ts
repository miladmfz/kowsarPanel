...
import { DailyActionControlWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/DailyActionControlWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-daily-action-control-list',
  templateUrl: './daily-action-control-list.component.html',
})
export class DailyActionControlListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(DailyActionControlWebApiService);
  ...
  title = signal('کنترل فروش روزانه')
  ClassName = signal('DailyActionControl')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetDailyActionControl(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.DailyActionControls)
