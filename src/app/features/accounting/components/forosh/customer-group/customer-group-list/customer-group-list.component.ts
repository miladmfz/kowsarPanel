...
import { CustomerGroupWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/CustomerGroupWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class CustomerGroupListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CustomerGroupWebApiService);
  ...
  title = signal('گروه مشتریان')
  ClassName = signal('CustomerGroup')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCustomerGroup(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CustomerGroups)
