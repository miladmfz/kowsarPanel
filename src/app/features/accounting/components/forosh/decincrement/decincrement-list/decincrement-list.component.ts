...
import { DecIncrementWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/DecIncrementWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-decincrement-list',
  templateUrl: './decincrement-list.component.html',
})
export class DecincrementListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(DecIncrementWebApiService);
  ...
  title = signal('اضافات و کسورات')
  ClassName = signal('DecIncrement')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetDecIncrement(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.DecIncrements)
