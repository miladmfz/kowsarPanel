...
import { GoodValueInPathGroupWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/GoodValueInPathGroupWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-goodvalueinpathgroup-list',
  templateUrl: './goodvalueinpathgroup-list.component.html',
})
export class GoodvalueinpathgroupListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodValueInPathGroupWebApiService);
  ...
  title = signal('قیمت گذاری کالاها در گروه مسیر')
  ClassName = signal('GoodValueinPathGroup')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodValueInPathGroup(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodValueInPathGroups)
