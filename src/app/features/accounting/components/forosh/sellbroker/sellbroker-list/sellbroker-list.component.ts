...
import { SellBrokerWebApiService } from 'src/app/features/accounting/services/ForoshWebApi/SellBrokerWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-sellbroker-list',
  templateUrl: './sellbroker-list.component.html',
})
export class SellbrokerListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(SellBrokerWebApiService);
  ...
  title = signal('مدیریت بازاریاب های فروش')
  ClassName = signal('SellBroker')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetSellBroker(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.SellBrokers)
