...
import { AccLevelWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccLevelWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-level-list',
  templateUrl: './acc-level-list.component.html',
})
export class AccLevelListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccLevelWebApiService);
  ...
  title = signal('معرفی سطوح حساب')
  ClassName = signal('AccLevel')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccLevel(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccLevels)
