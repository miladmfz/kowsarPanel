...
import { AccKolWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccKolWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-kol-list',
  templateUrl: './acc-kol-list.component.html',
})
export class AccKolListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccKolWebApiService);
  ...
  title = signal('معرفی حسابهای کل')
  ClassName = signal('AccKol')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccKol(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccKols)
