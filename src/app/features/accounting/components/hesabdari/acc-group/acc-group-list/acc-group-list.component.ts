...
import { AccGroupWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccGroupWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-group-list',
  templateUrl: './acc-group-list.component.html',
})
export class AccGroupListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccGroupWebApiService);
  ...
  title = signal('معرفی گروه حسابها')
  ClassName = signal('AccGroup')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccGroup(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccGroups)
