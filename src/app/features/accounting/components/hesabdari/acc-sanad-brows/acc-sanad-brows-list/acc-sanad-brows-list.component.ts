...
import { AccSanadBrowsWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccSanadBrowsWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-sanad-brows-list',
  templateUrl: './acc-sanad-brows-list.component.html',
})
export class AccSanadBrowsListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccSanadBrowsWebApiService);
  ...
  title = signal('مرور حسابها')
  ClassName = signal('AccSanadBrowse')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccSanadBrows(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccSanadBrows)
