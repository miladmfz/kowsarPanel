...
import { AccTafzilyWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccTafzilyWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-tafzily-list',
  templateUrl: './acc-tafzily-list.component.html',
})
export class AccTafzilyListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccTafzilyWebApiService);
  ...
  title = signal('معرفی حسابهای تفضیلی')
  ClassName = signal('AccTafzily')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccTafzily(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccTafzilys)
