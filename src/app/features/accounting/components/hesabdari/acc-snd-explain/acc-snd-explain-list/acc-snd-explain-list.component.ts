...
import { AccSndExplainWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccSndExplainWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-snd-explain-list',
  templateUrl: './acc-snd-explain-list.component.html',
})
export class AccSndExplainListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccSndExplainWebApiService);
  ...
  title = signal('شرح اسناد')
  ClassName = signal('AccSndExplain')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccSndExplain(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccSndExplains)
