...
import { AccAccountsWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccAccountsWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-accounts-list',
  templateUrl: './acc-accounts-list.component.html',
})
export class AccAccountsListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccAccountsWebApiService);
  ...
  title = signal('جابجایی حساب')
  ClassName = signal('AccAccounts')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccAccounts(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccAccountss)
