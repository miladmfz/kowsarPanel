...
import { VwCentralBrowsWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/VwCentralBrowsWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-vwcentral-brows-list',
  templateUrl: './vwcentral-brows-list.component.html',
})
export class VwcentralBrowsListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(VwCentralBrowsWebApiService);
  ...
  title = signal('مرور حساب اشخاص و شرکتها')
  ClassName = signal('vwCentralBrowse')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCentralBrowse(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CentralBrowses)
