...
import { FactorAccountWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/FactorAccountWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-factor-account-list',
  templateUrl: './factor-account-list.component.html',
})
export class FactorAccountListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(FactorAccountWebApiService);
  ...
  title = signal('طرف حساب های اتوماتیک')
  ClassName = signal('FactorAccount')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetFactorAccount(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.FactorAccounts)
