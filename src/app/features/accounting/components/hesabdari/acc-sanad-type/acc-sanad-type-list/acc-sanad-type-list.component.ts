...
import { AccSanadTypeWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccSanadTypeWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-sanad-type-list',
  templateUrl: './acc-sanad-type-list.component.html',
})
export class AccSanadTypeListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccSanadTypeWebApiService);
  ...
  title = signal('صدور سند')
  ClassName = signal('AccSanadType')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccSanadType(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccSanadTypes)
