...
import { AccMoeenWebApiService } from 'src/app/features/accounting/services/HesabdariWebApi/AccMoeenWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-acc-moeen-list',
  templateUrl: './acc-moeen-list.component.html',
})
export class AccMoeenListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AccMoeenWebApiService);
  ...
  title = signal('معرفی حسابهای معین')
  ClassName = signal('AccMoeen')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAccMoeen(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AccMoeens)
