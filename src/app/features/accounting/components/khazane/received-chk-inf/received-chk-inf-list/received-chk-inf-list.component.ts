...
import { ReceivedChkInfWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/ReceivedChkInfWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-received-chk-inf-list',
  templateUrl: './received-chk-inf-list.component.html',
})
export class ReceivedChkInfListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(ReceivedChkInfWebApiService);
  ...
  title = signal('اعلامیه وصول چک')
  ClassName = signal('ReceivedChkInf')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetReceivedChkInf(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.ReceivedChkInfs)
