...
import { KhazanePaymentWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/KhazanePaymentWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-khazane-payment-list',
  templateUrl: './khazane-payment-list.component.html',
})
export class KhazanePaymentListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(KhazanePaymentWebApiService);
  ...
  title = signal('پرداخت')
  ClassName = signal('KhazanehPayment')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetKhazanehPayment(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.KhazanehPayments)
