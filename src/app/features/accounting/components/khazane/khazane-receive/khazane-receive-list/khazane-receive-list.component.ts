...
import { KhazaneReceiveWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/KhazaneReceiveWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-khazane-receive-list',
  templateUrl: './khazane-receive-list.component.html',
})
export class KhazaneReceiveListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(KhazaneReceiveWebApiService);
  ...
  title = signal('دریافت')
  ClassName = signal('KhazanehReceive')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetKhazanehReceive(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.KhazanehReceives)
