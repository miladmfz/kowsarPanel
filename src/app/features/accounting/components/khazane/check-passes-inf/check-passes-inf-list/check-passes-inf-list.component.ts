...
import { CheckPassesInfWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/CheckPassesInfWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-check-passes-inf-list',
  templateUrl: './check-passes-inf-list.component.html',
})
export class CheckPassesInfListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CheckPassesInfWebApiService);
  ...
  title = signal('اعلامیه پاس شدن چک')
  ClassName = signal('CheckPassedInf')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCheckPassedInf(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CheckPassedInfs)
