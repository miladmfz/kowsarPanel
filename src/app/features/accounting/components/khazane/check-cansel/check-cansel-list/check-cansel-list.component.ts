...
import { CheckCanselWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/CheckCanselWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-check-cansel-list',
  templateUrl: './check-cansel-list.component.html',
})
export class CheckCanselListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CheckCanselWebApiService);
  ...
  title = signal('ابطال چک')
  ClassName = signal('CheckCancel')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCheckCancel(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CheckCancels)
