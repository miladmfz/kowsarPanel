...
import { BankWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/BankWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class BankListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(BankWebApiService);
  ...
  title = signal('بانک ها')
  ClassName = signal('bank')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetBank(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Banks)
