...
import { CashWebApiService } from 'src/app/features/accounting/services/KhazaneWebApi/CashWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class CashListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CashWebApiService);
  ...
  title = signal('صندوق ها')
  ClassName = signal('cash')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCash(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Cashs)
