...
import { CloseAccSenarioWebApiService } from 'src/app/features/accounting/services/KharidWebApi/CloseAccSenarioWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-close-acc-senario-list',
  templateUrl: './close-acc-senario-list.component.html',
})
export class CloseAccSenarioListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(CloseAccSenarioWebApiService);
  ...
  title = signal('انتقال ارزش ریالی انبار')
  ClassName = signal('CloseAccSenario')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetCloseAccSenario(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.CloseAccSenarios)
