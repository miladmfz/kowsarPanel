...
import { ReturnPurchaseWebApiService } from 'src/app/features/accounting/services/KharidWebApi/ReturnPurchaseWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class ReturnPurchaseListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(ReturnPurchaseWebApiService);
  ...
  title = signal('مرجوع از خرید')
  ClassName = signal('ReturnPurchase')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetReturnPurchase(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.ReturnPurchases)
