...
import { VendorMandehWebApiService } from 'src/app/features/accounting/services/KharidWebApi/VendorMandehWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-vendor-mandeh-list',
  templateUrl: './vendor-mandeh-list.component.html',
})
export class VendorMandehListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(VendorMandehWebApiService);
  ...
  title = signal('حساب فروشندگان')
  ClassName = signal('VendorMandeh')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetVendorMandeh(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.VendorMandehs)
