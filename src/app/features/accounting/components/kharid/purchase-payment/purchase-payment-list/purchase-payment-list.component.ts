...
import { PurchasePaymentWebApiService } from 'src/app/features/accounting/services/KharidWebApi/PurchasePaymentWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class PurchasePaymentListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(PurchasePaymentWebApiService);
  ...
  title = signal('پرداخت')
  ClassName = signal('PurchasePayment')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetPurchasePayment(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.PurchasePayments)
