...
import { ExtendedBarcodeRptWebApiService } from 'src/app/features/accounting/services/KharidWebApi/ExtendedBarcodeRptWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-extended-barcode-rpt-list',
  templateUrl: './extended-barcode-rpt-list.component.html',
})
export class ExtendedBarcodeRptListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(ExtendedBarcodeRptWebApiService);
  ...
  title = signal('ایجاد بارکد')
  ClassName = signal('ExtendedBarCoderpt')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodBarCode(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodBarCodes)
