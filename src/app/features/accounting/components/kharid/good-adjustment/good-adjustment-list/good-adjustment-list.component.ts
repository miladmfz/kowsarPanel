...
import { GoodAdjustmentWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodAdjustmentWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-adjustment-list',
  templateUrl: './good-adjustment-list.component.html',
})
export class GoodAdjustmentListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodAdjustmentWebApiService);
  ...
  title = signal('تسویه مبادله')
  ClassName = signal('GoodAdjustment')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodAdjustment(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodAdjustments)
