...
import { GoodRequestWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodRequestWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-request-list',
  templateUrl: './good-request-list.component.html',
})
export class GoodRequestListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodRequestWebApiService);
  ...
  title = signal('درخواست نیاز کالا')
  ClassName = signal('GoodRequest')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodRequest(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodRequests)
