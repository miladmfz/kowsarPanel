...
@Component({
  selector: 'app-good-property',
  templateUrl: './good-property.component.html',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class GoodPropertyComponent implements OnInit {
  @Input() GoodCode = ""
  ...
  setinitial_property() {
    ...
    (this.KowsarTemplate.get('Good') as FormArray).push(this.GoodToProperty);
  ...
  LoadData_property() {
    ...
    this.repo.GetGood_Propertys(this.Code()).subscribe((data: any) => {
      ...
      if (data.response.Errormessage.length > 0) {
        ...
        this.GoodTypeStr.set(data.Goods[0].GoodType)
        ...
        this.repo.GetProperty(this.propertyDto.value).subscribe((data: any) => {
          ...
          this.Propertys.set(data.Propertys)
