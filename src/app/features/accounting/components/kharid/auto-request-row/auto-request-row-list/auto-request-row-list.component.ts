...
import { AutoRequestRowWebApiService } from 'src/app/features/accounting/services/KharidWebApi/AutoRequestRowBankWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-auto-request-row-list',
  templateUrl: './auto-request-row-list.component.html',
})
export class AutoRequestRowListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(AutoRequestRowWebApiService);
  ...
  title = signal('معرفی کالاهای سفارش اتوماتیک')
  ClassName = signal('AutoRequestRow')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetAutoRequest(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.AutoRequests)
