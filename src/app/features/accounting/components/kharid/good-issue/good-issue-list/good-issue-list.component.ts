...
import { GoodIssueWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodIssueWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-issue-list',
  templateUrl: './good-issue-list.component.html',
})
export class GoodIssueListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodIssueWebApiService);
  ...
  title = signal('حواله انبار')
  ClassName = signal('GoodIssue')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodIssue(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodIssues)
