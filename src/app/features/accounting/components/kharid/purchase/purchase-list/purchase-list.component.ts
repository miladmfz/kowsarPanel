...
import { PurchaseWebApiService } from 'src/app/features/accounting/services/KharidWebApi/PurchaseWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class PurchaseListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(PurchaseWebApiService);
  ...
  title = signal('فاکتور خرید')
  ClassName = signal('Purchase')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetPurchase(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Purchases)
