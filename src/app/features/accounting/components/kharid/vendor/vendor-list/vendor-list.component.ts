...
import { VendorWebApiService } from 'src/app/features/accounting/services/KharidWebApi/VendorWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class VendorListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(VendorWebApiService);
  ...
  title = signal('فروشندگان')
  ClassName = signal('Vendor')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetVendor(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Vendors)
