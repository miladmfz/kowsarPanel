...
import { LocationWebApiService } from 'src/app/features/accounting/services/KharidWebApi/LocationWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class LocationListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(LocationWebApiService);
  ...
  GetData() {
    ...
    this.repo.GetLocation(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.Locations)
