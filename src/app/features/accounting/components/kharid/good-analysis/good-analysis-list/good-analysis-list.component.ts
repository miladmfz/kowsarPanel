...
import { GoodAnalysisWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodAnalysisWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-analysis-list',
  templateUrl: './good-analysis-list.component.html',
})
export class GoodAnalysisListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodAnalysisWebApiService);
  ...
  title = signal('تجزیه و ترکیب کالا')
  ClassName = signal('GoodAnalysis')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodAnalysis(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodAnalysiss)
