...
import { RepLogLinkWebApiService } from 'src/app/features/accounting/services/KharidWebApi/RepLogLinkWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-reploglink-list',
  templateUrl: './reploglink-list.component.html',
})
export class ReploglinkListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(RepLogLinkWebApiService);
  ...
  title = signal('کالا های سایت')
  ClassName = signal('RepLogLink')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetRepLogLink(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.RepLogLinks)
