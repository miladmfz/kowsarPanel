...
import { GoodReceiptWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodReceiptWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-receipt-list',
  templateUrl: './good-receipt-list.component.html',
})
export class GoodReceiptListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodReceiptWebApiService);
  ...
  title = signal('رسید انبار')
  ClassName = signal('GoodReceipt')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodReceipt(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodReceipts)
