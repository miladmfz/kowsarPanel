...
import { SamaneGoodWebApiService } from 'src/app/features/accounting/services/KharidWebApi/SamaneGoodWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-samane-good-list',
  templateUrl: './samane-good-list.component.html',
})
export class SamaneGoodListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(SamaneGoodWebApiService);
  ...
  title = signal('کالاهای سامانه جامع انبار')
  ClassName = signal('SamaneGood')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetSamaneGood(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.SamaneGoods)
