...
import { StackTransferWebApiService } from 'src/app/features/accounting/services/KharidWebApi/StackTransferWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-stack-transfer-list',
  templateUrl: './stack-transfer-list.component.html',
})
export class StackTransferListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(StackTransferWebApiService);
  ...
  title = signal('لنتقال انبار به انبار')
  ClassName = signal('StackTransfer')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetStackTransfer(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.StackTransfers)
