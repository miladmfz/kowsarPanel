...
import { StackEnumerationWebApiService } from 'src/app/features/accounting/services/KharidWebApi/StackEnumerationWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-stack-enumeration-list',
  templateUrl: './stack-enumeration-list.component.html',
})
export class StackEnumerationListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(StackEnumerationWebApiService);
  ...
  title = signal('انبارگردانی')
  ClassName = signal('StackEnumeration')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetStackEnumeration(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.StackEnumerations)
