...
import { PersonJobWebApiService } from 'src/app/features/accounting/services/KharidWebApi/PersonJobWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-personjob-list',
  templateUrl: './personjob-list.component.html',
})
export class PersonjobListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(PersonJobWebApiService);
  ...
  title = signal('مدیریت ارتباط اشخاص و کالاها')
  ClassName = signal('PersonJob')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetPersonJob(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.PersonJobs)
