...
import { PurchaseOrderWebApiService } from 'src/app/features/accounting/services/KharidWebApi/PurchaseOrderWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class PurchaseOrderListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(PurchaseOrderWebApiService);
  ...
  title = signal('سفارش خرید')
  ClassName = signal('PurchaseOrder')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetPurchaseOrder(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.PurchaseOrders)
