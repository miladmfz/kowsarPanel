...
import { RequestOrderWebApiService } from 'src/app/features/accounting/services/KharidWebApi/RequestOrderWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
...
export class RequestOrderListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(RequestOrderWebApiService);
  ...
  title = signal('مدیریت درخواست نیاز و تامین کالا')
  ClassName = signal('RequestOrder')
  records = signal<any[]>([])
