...
import { GoodSerialFrameWebApiService } from 'src/app/features/accounting/services/KharidWebApi/GoodSerialFrameWebApi.service';
@Component({
  standalone: true,
  imports: [
    CommonModule,
  ...
  selector: 'app-good-serial-frame-list',
  templateUrl: './good-serial-frame-list.component.html',
})
export class GoodSerialFrameListComponent extends AgGridBaseComponent
  implements OnInit, OnDestroy {
  private readonly router = inject(Router);
  private readonly notificationService = inject(NotificationService);

  private readonly repo = inject(GoodSerialFrameWebApiService);
  ...
  title = signal('موجودی اول دوره')
  ClassName = signal('GoodSerialFrame')
  records = signal<any[]>([])
  ...
  GetData() {
    ...
    this.repo.GetGoodSerial(this.EditForm_Search.value)
      .subscribe((data: any) => {

        this.records.set(data.GoodSerials)
