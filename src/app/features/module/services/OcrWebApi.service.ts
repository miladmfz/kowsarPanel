...
export class OcrWebApiService {
    ...
    OcrFactorList(Searchtarget: string, SourceFlag: string): Observable<any[]> {
        return this.withLoading(this.client.post<any[]>(this.baseUrl + "OcrFactorList", { Searchtarget, SourceFlag }, { headers: this.headerService.headers }))
    }
    ...
    GetOcrPanel(StartDate: string, EndDate: string, State: string): Observable<any[]> {
        ...
        return this.withLoading(this.client.get<any[]>(this.baseUrl + "GetOcrPanel", { headers: this.headerService.headers, params: params }))
    }
