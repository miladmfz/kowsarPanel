...
export class SalarysummaryListComponent extends AgGridBaseComponent
  ...
  // دیتاها
  records = signal<any[]>([])
  records_MonthSummarys = signal<any[]>([])
  ...
  ngOnInit(): void {
    ...
    this.columnDefs2 = [
      ...
      { field: 'Sal', headerName: 'سال', cellClass: 'text-center', minWidth: 100 },
      { field: 'Mah', headerName: 'ماه', cellClass: 'text-center', minWidth: 100 },
      { field: 'TotalDays', headerName: 'تعداد روز', cellClass: 'text-center', minWidth: 100 },
      { field: 'WorkingDays', headerName: 'روز کاری', cellClass: 'text-center', minWidth: 100 },
      { field: 'HolidayDays', headerName: 'روز تعطیل', cellClass: 'text-center', minWidth: 100 },
      { field: 'WorkingHoursInMonth', headerName: 'ساعت موظفی', cellClass: 'text-center', minWidth: 100 },
  ...
  getList() {
    ...
    this.repo.GetSalarySummary(this.EditForm_SearchTarget.value).subscribe({
      next: (data: any) => {
        ...
        this.records.set(data?.SalarySummarys ?? [])
  ...
  ShowModalAddSalary(): void {
    ...
    this.repo.GetMonthSummary(this.EditForm_SearchTarget.value).subscribe({
      next: (data: any) => {
        ...
        this.records_MonthSummarys.set(data?.MonthSummarys ?? [])
  ...
  @ViewChild('salarymodal', { static: false }) salarymodal!: ElementRef<HTMLDivElement>;
  @ViewChild('editWorkingEmployeemodal', { static: false }) editWorkingEmployeemodal!: ElementRef<HTMLDivElement>;
  @ViewChild('addsalarymodal', { static: false }) addsalarymodal!: ElementRef<HTMLDivElement>;
  private readonly renderer = inject(Renderer2);



  salary_dialog_show(): void {
    const modal = this.salarymodal?.nativeElement;
    if (!modal) return;
  ...
  WorkingEmploye_dialog_show(): void {
    const modal = this.editWorkingEmployeemodal?.nativeElement;
    if (!modal) return;
    this.renderer.addClass(modal, 'show');
    this.renderer.setStyle(modal, 'display', 'block');
    this.renderer.setAttribute(modal, 'aria-modal', 'true');
  ...
  WorkingEmploye_dialog_close(): void {
    const modal = this.editWorkingEmployeemodal?.nativeElement;
    if (!modal) return;
    this.renderer.removeClass(modal, 'show');
    this.renderer.setStyle(modal, 'display', 'none');
  ...
  addsalary_dialog_show(): void {
    const modal = this.addsalarymodal?.nativeElement;
    if (!modal) return;
    this.renderer.addClass(modal, 'show');
    this.renderer.setStyle(modal, 'display', 'block');
    this.renderer.setAttribute(modal, 'aria-modal', 'true');
