...
export class AutletterChatComponent implements OnInit, AfterViewInit {
  ...
  GetAutConversation() {
    ...
    this.repo.GetAutConversation(this.LetterRef()).subscribe({
      next: (res: any) => {
        ...
        this.repo.ConversationSeen(this.CentralRef(), this.LetterRef()).subscribe({
          next: (res: any) => { }
        });
  ...
  SendMessage() {
    ...
    const cleanMessage_From_Digits = this.convertSpecialCharacters(desc);

    const cleanMessage_From_number = this.convertToEnglishDigits(cleanMessage_From_Digits);



    this.repo.Conversation_Insert(this.LetterRef(), this.CentralRef(), cleanMessage_From_Digits)
      .subscribe({
        next: () => {
