...
import { CellActionAutletterKowsarList } from '../autletter-kowsar-list/cell-action-autletter-kowsar-list';
import { CellStateAutletterKowsar } from '../autletter-kowsar-list/cell-state-autletter-kowsar';
...
export class AutLetterMineComponent extends AgGridBaseComponent implements OnInit {
  ...
  loadMyTickets(): void {
    ...
    const PersonInfoCode = this.session.personInfoRef;
    ...
    this.repo.GetAutLetterListByCentral(this.EditForm_autletter.value).subscribe({
      next: (data: any) => {
