...
@Component({
  ...
  templateUrl: './internal-good-edit.component.html',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class InternalGoodEditComponent extends AgGridBaseComponent implements OnInit {
      ...
      if (Object.keys(this.changedValues).length > 0) {
        ...
        this.repo.GoodCrudService(this.JsonForm.value).subscribe((data: any) => {
          ...
          if (result.Goods && result.Goods[0].ErrMessage === "") {
            ...
            this.getDetails()
            this.router.navigate(['/internal/internal-good-edit', result.Goods[0].GoodCode]);
          } else {
  ...
  GotoSuggestion(simillar_good): void {
    ...
    this.router.navigate(['/internal/internal-good-edit', simillar_good.GoodCode]);
