...
export class PrefactorEditComponent extends AgGridBaseComponent
  ...
  override ngOnInit(): void {
    ...
    if (this.PreFactorCode.length > 0) {
      this.GetFactor();
    } else {
      this.getdate();
    }
    ...
    this.columnDefs3 = [
      {
        field: 'عملیات',
        pinned: 'left',
        cellRenderer: CellActionPreFactorCustomerEdit,
        cellRendererParams: {
          editUrl: '/support/letter-detail',
        },
        width: 80,
  ...
  // #region Func
  NewPreFactor() {
    this.router.navigate(['/factor/prefactor-edit']);
  }
  ...
  deleteFactor() {
    if (this.records_factorrows && this.records_factorrows.length > 0) {
      this.notificationService.error('این پیش فاکتور دارای اقلام می باشد');
    } else {
